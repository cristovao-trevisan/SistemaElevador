  ...
  for (;;) {  
    ...
    if(evt.status == osEventMessage)
    {
      ...
      if (message->msg[1] == 'E')
      {
        ...
        if (valueLeft > valueCenter && valueLeft > valueRight) {
            REQUEST_FROM(elevatorStatusLeft, andar);
        }
        else if (valueCenter > valueRight) {
            REQUEST_FROM(elevatorStatusCenter, andar);
        }
        else {
            REQUEST_FROM(elevatorStatusRight, andar);
        }
  ...
  for (;;) {   
    ...
    if(evt.status == osEventMessage)
    {
      ...
      else if(key >= '0' && key <= '9')
      {
        ...
        current_floor = atoi((char*)message->msg);
    ...
    if(cont == 50000) {
      cont = 0;
      switch(elevatorStatusLeft.state) {
        case(Stopped):
          write2USART("ep", 2);
          break;
        case(GoingUp):
...
void rightElevator (void const *argument) {
  ...
  for (;;) {   
    ...
    if(cont == 50000) {
      cont = 0;
      switch(elevatorStatusRight.state) {
        case(Stopped):
          write2USART("dp", 2);
          break;
        case(GoingUp):
...
void centerElevator (void const *argument) {
  ...
  for (;;) {   
    ...
    if(cont == 50000) {
      cont = 0;
      switch(elevatorStatusCenter.state) {
        case(Stopped):
          write2USART("cp", 2);
          break;
        case(GoingUp):
...
int evaluation(ElevatorStatus elevator, int requestedFloor, int buttonUp){
	...
	if ((elevator.state == GoingUp && distancePoints > 0) || (elevator.state == GoingDown && distancePoints < 0) || (elevator.state == Stopped))
		directionPoints = 1;
